...
public class Shuffler {
	...
	public static void perfectShuffle(int[] values) {
		...
		int[] half2 = new int[VALUE_COUNT / 2];
		
		
		if(values.length % 2 == 0) {
			System.arraycopy(values, 0, half1, 0, VALUE_COUNT / 2);
			System.arraycopy(values, (VALUE_COUNT) / 2, half2, 0, VALUE_COUNT / 2);
		} else {
			System.arraycopy(values, 0, half1, 0, (VALUE_COUNT) / 2);
			System.arraycopy(values, (VALUE_COUNT + 1) / 2, half2, 0, VALUE_COUNT / 2);
		}
		...
		int limit = (VALUE_COUNT % 2 == 0) ? VALUE_COUNT / 2 : (VALUE_COUNT + 1) / 2;
		
		for (int i = 0; i < limit; i++) {
			
				shuffled[2 * i] = half1[i];

				shuffled[(2 * i) + 1] = half2[i];
